        ...
        <form action="{{ route('admin.updateHotel', $hotel->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <div id="image_urls_container">
                        @if(old('image_urls', $hotel->images->pluck('image_path')->all()))
                            @foreach(old('image_urls', $hotel->images->pluck('image_path')->all()) as $index => $imageUrl)
                                @if(!empty($imageUrl)) {{-- لا تعرض حقلًا فارغًا إذا كان old يحتوي على قيمة فارغة --}}
                                <div class="input-group mb-2 image-url-input-group">
                                    <input type="url" name="image_urls[]" class="form-control @error('image_urls.'.$index) is-invalid @enderror" placeholder="https://example.com/image.jpg" value="{{ $imageUrl }}">
                                    <button type="button" class="btn btn-danger remove-image-url-btn" @if($loop->first && count(old('image_urls', $hotel->images->pluck('image_path')->all())) == 1 && empty(old('image_urls', $hotel->images->pluck('image_path')->all())[0])) style="display: none;" @elseif(count(old('image_urls', $hotel->images->pluck('image_path')->all())) <= 1 && empty($imageUrl) && $loop->first) style="display: none;" @endif >-</button>
                                </div>
                                @endif
                            @endforeach
                             {{-- إذا كانت المصفوفة فارغة تمامًا أو تحتوي على عنصر فارغ واحد فقط، أضف حقلًا فارغًا واحدًا --}}
                            @if(empty(old('image_urls', $hotel->images->pluck('image_path')->all())) || (count(old('image_urls', $hotel->images->pluck('image_path')->all())) == 1 && empty(old('image_urls', $hotel->images->pluck('image_path')->all())[0])))
                                <div class="input-group mb-2 image-url-input-group">
                                    <input type="url" name="image_urls[]" class="form-control @error('image_urls.*') is-invalid @enderror" placeholder="https://example.com/image.jpg">
                                    <button type="button" class="btn btn-danger remove-image-url-btn" style="display: none;">-</button>
                                </div>
                            @endif
                        @else
                    ...
                    <button type="button" id="add_image_url_btn" class="btn btn-success btn-sm mt-2">+ إضافة رابط صورة أخرى</button>
                    <small class="form-text text-muted d-block mt-1">أدخل روابط URL كاملة للصور. سيتم استبدال جميع الصور الحالية بالروابط الجديدة. اترك جميع الحقول فارغة لحذف كل الصور.</small>
                    @error('image_urls.*')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
    
                    {{-- عرض الصور الحالية (يمكنك إبقاؤه أو إزالته حسب الرغبة) --}}
                    @if ($hotel->images->count() > 0 && !old('image_urls')) {{-- لا تعرض إذا كان هناك old data --}}
                        <div class="mt-3">
