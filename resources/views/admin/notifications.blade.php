...
<style>
    ...
    .notification-type.security-alert { background: var(--danger); }
    .notification-type.land-trip { background: var(--info); }
    .notification-type.booking { background: var(--primary); }
    .notification-type.payment { background: var(--success); }
    .notification-type.login { background: var(--warning); color: #212529; }
    .notification-type.delete { background: var(--danger); }
    ...
    @media (max-width: 768px) {
        .notifications-container { padding: 1rem 0.5rem; }
        .page-title { font-size: 1.3rem; }
        .notification-content { padding: 0.7rem 0.5rem; }
        .notification-header { flex-direction: column; align-items: flex-start; gap: 0.3rem; }
        .notification-footer { flex-direction: column; align-items: flex-start; gap: 0.7rem; }
    }
</style>
@endpush
        ...
        <div class="filter-wrapper">
            <div class="filter-scroll" id="filter-scroll">
                <div class="filter-buttons">
                    ...
                    <a href="javascript:void(0)" data-url="{{ route('admin.notifications', ['filter' => 'bookings']) }}" data-filter="bookings"
                    ...
                    <a href="javascript:void(0)" data-url="{{ route('admin.notifications', ['filter' => 'payments']) }}" data-filter="payments"
                    ...
                    <a href="javascript:void(0)" data-url="{{ route('admin.notifications', ['filter' => 'availabilities']) }}" data-filter="availabilities"
                        class="filter-btn btn {{ $currentFilter == 'availabilities' ? 'active btn-info' : 'btn-outline-info' }}">
                        <i class="fas fa-calendar-alt"></i> الإتاحات
                    </a>
                    
                    <a href="javascript:void(0)" data-url="{{ route('admin.notifications', ['filter' => 'land-trips']) }}" data-filter="land-trips"
                        class="filter-btn btn {{ $currentFilter == 'land-trips' ? 'active btn-info' : 'btn-outline-info' }}">
                        <i class="fas fa-bus-alt"></i> الرحلات البرية
                    </a>
                    
                    <a href="javascript:void(0)" data-url="{{ route('admin.notifications', ['filter' => 'logins']) }}" data-filter="logins"
            ...
            <div id="notifications-content">
                ...
                @forelse($notifications as $notification)
                    ...
                    @php
                        ...
                        elseif (in_array($notification->type, ['إضافة حجز', 'تأكيد حجز', 'تعديل حجز', 'إلغاء حجز'])) {
                            $itemClass .= ' booking';
                            $typeClass = 'booking';
                        ...
                        <div class="notification-content">
                            <div class="notification-header">
                                <div class="notification-author">
                                    ...
                                    <strong class="text-primary">{{ $notification->user ? $notification->user->name : 'غير معروف' }}</strong>
                                </div>
                            ...
                            <div class="notification-footer">
                                <div class="notification-time">
                                    ...
                                    <span title="{{ $notification->created_at->format('Y-m-d H:i:s') }}">{{ $notification->created_at->diffForHumans() }}</span>
                                </div>
                                
                                @if (!$notification->is_read)
                                    <button class="btn btn-sm action-btn read-btn mark-read" data-id="{{ $notification->id }}">
...
$(function() {
    ...
    $('#mark-all-read-btn').on('click', function() {
        ...
        $.post('/admin/notifications/mark-all-read', {
            ...
            $btn.prop('disabled', false).html('<i class="fas fa-check-double"></i> تحديد الكل كمقروء');
            Swal.fire('خطأ', 'حدث خطأ أثناء تحديث حالة الإشعارات، يرجى المحاولة مرة أخرى.', 'error');
        });
    });
    ...
    $(document).on('click', '.mark-read', function() {
        ...
        $.post(`/admin/notifications/${id}/read`, { // <-- هنا التعديل
            ...
            Swal.fire('خطأ', 'حدث خطأ أثناء تحديث حالة الإشعار، يرجى المحاولة مرة أخرى.', 'error');
        });
    });
