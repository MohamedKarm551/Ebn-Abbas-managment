                    ...
                    <tbody>
                        @forelse($transactions as $transaction)
                            <tr class="transaction-row {{ $transaction->type }}">
                                ...
                                <td>
                                    <span
                                        class="badge currency-badge bg-{{ $transaction->currency == 'SAR' ? 'primary' : ($transaction->currency == 'KWD' ? 'success' : 'info') }}">
                                        {{ $transaction->currency }}
                                    </span>
                                </td>
                                <td>
                                    <span
                                        class="badge bg-{{ $transaction->type == 'deposit' ? 'success' : ($transaction->type == 'withdrawal' ? 'danger' : ($transaction->type == 'transfer' ? 'warning' : 'info')) }}">
                                        @if ($transaction->type == 'deposit')
                                            <i class="fas fa-arrow-up me-1"></i> إيداع (+)
                                        @elseif($transaction->type == 'withdrawal')
                                            <i class="fas fa-arrow-down me-1"></i> سحب (-)
                                        @elseif($transaction->type == 'transfer')
                                            <i class="fas fa-exchange-alt me-1"></i> تحويل (-)
                                        @else
                                            <i class="fas fa-circle me-1"></i> {{ $transaction->type_arabic }}
                                        @endif
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="action-buttons d-flex gap-1">
                                        ...
                                        <form action="{{ route('admin.transactions.destroy', $transaction) }}"
                                            method="POST" class="d-inline" id="delete-form-{{ $transaction->id }}"
                        ...
                        <div class="card transaction-card {{ $transaction->type }} h-100">
                            ...
                            <div class="card-body">
                                ...
                                <div class="text-center mb-3">
                                    <h4
                                        class="mb-1 {{ $transaction->type == 'deposit' ? 'text-success' : 'text-danger' }}">
                                        {{ $transaction->type == 'withdrawal' ? '-' : '+' }}{{ number_format($transaction->amount, 2) }}
                                        <small class="text-muted">{{ $transaction->currency_symbol }}</small>
                                    </h4>
                                    <span
                                        class="badge bg-{{ $transaction->currency == 'SAR' ? 'primary' : ($transaction->currency == 'KWD' ? 'success' : 'info') }}">
                                        {{ $transaction->currency }}
                                    </span>
                                </div>
