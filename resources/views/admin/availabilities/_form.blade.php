...
<div x-data="availabilityForm({
 ...
 'allotment' => $art->allotment])) : '[]' }},
    ...
    <div class="row g-3">
        <div class="col-md-4">
            ...
            <select name="hotel_id" id="hotel_id" class="form-select select2 @error('hotel_id') is-invalid @enderror"
                required data-placeholder="اختر الفندق...">
        ...
        <div class="col-md-4">
            ...
            <select name="agent_id" id="agent_id" class="form-select select2 @error('agent_id') is-invalid @enderror"
                required data-placeholder="اختر جهة الحجز...">
        ...
        <div class="col-md-4">
            ...
            <select name="employee_id" id="employee_id"
                ...
                @foreach ($employees as $employee)
                    <option value="{{ $employee->id }}" {{-- Default to logged-in user ONLY on create, otherwise use existing or old value --}}
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="start_date" id="start_date"
                ...
                value="{{ old('start_date', $availability?->start_date?->format('Y-m-d')) }}" required
                data-date-format="dd/mm/yyyy" placeholder="dd/mm/yyyy" autocomplete="off"> {{-- Use text type for datepicker --}}
            @error('start_date')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="end_date" id="end_date"
                ...
                value="{{ old('end_date', $availability?->end_date?->format('Y-m-d')) }}" required
                data-date-format="dd/mm/yyyy" placeholder="dd/mm/yyyy" autocomplete="off"> {{-- Use text type for datepicker --}}
            @error('end_date')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <div class="col-md-6">
            ...
            <input type="number" class="form-control @error('min_nights') is-invalid @enderror" id="min_nights"
                name="min_nights" value="{{ old('min_nights', $availability ? $availability->min_nights : '') }}"
                min="1" placeholder="مثال: 3">
            <small class="form-text text-muted">اتركه فارغًا إذا لم يكن هناك حد أدنى لعدد الليالي.</small>
            @error('min_nights')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            ...
            <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                <option value="active" {{ old('status', $availability?->status) == 'active' ? 'selected' : '' }}>نشطة
                </option>
                <option value="inactive" {{ old('status', $availability?->status) == 'inactive' ? 'selected' : '' }}>
                    غير نشطة</option>
    ...
    @error('room_types.*')
        <div class="alert alert-danger">يرجى مراجعة بيانات أسعار الغرف المدخلة والتأكد من عدم تكرار نوع الغرفة.</div>
    @enderror
    ...
    <div id="room-types-container" class="mb-3">
        ...
        <template x-for="(room, index) in roomTypes" :key="index">
            <div class="row g-3 align-items-center border-bottom pb-3 mb-3">
                ...
                <div class="col-md-3">
                    <label :for="'room_type_id_' + index" class="form-label">نوع الغرفة <span
                            class="text-danger">*</span></label>
                    <select class="form-select" :name="'room_types[' + index + '][room_type_id]'"
                        x-model="room.room_type_id" required @change="checkDuplicate(index)">
                        <option value="" disabled>اختر نوع الغرفة</option>
                        <template x-for="(name, id) in allRoomTypes" :key="id">
                            <option :value="id" x-text="name" :selected="room.room_type_id == id"></option>
                        </template>
                    </select>
                ...
                <div class="col-md-2">
                    <label :for="'cost_price_' + index" class="form-label">سعر التكلفة <span
                            class="text-danger">*</span></label>
                ...
                <div class="col-md-2">
                    <label :for="'sale_price_' + index" class="form-label">سعر البيع <span
                            class="text-danger">*</span></label>
                    <input type="number" step="0.01" class="form-control"
...
<script>
    function availabilityForm(config) {
        return {
            ...
            roomTypes: config.initialRoomTypes.map(rt => ({
                ...rt,
                duplicate: false
            })) || [],
            ...
            init() {
                ...
                this.$watch('roomTypes', () => this.validateDuplicates(), {
                    deep: true
                });
