                        ...
                        <tbody>
                            @forelse($profitsByEmployee as $employeeId => $data)
                                <tr>
                                    ...
                                    <td>{{ number_format($data['total_profit'], 2) }} KWD</td>
                                    <td>
                                        {{ number_format($data['total_employee_profit'], 2) }} EGP
                            ...
                            @if (count($profitsByEmployee))
                                <tr class="table-secondary fw-bold">
                                    ...
                                    <td>{{ number_format($totalCompanyProfit, 2) }} KWD</td>
                                    <td>{{ number_format($totalEmployeeProfit, 2) }} EGP</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">
                        تفاصيل تقارير {{ $data['employee']->name ?? 'غير معروف' }} ({{ $data['reports_count'] }})
                    </h6>
                </div>
                <div class="card-body">
                    @php
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @forelse($employeeReports as $report)
                                    <tr>
                                        <td>{{ $loop->iteration }} : {{ $report->report_date->format('Y-m-d') }}</td>
                                        <td>{{ $report->client_name }}</td>
                                        <td>{{ $report->company_name ?? '-' }}</td>
                                        <td>{{ number_format($report->grand_total_profit, 2) }} {{ $report->currency }}
                                        </td>
                                        <td class="text-success">
                                            {{ number_format($report->employee_profit, 2) }}
                                            {{ $report->employee_profit_currency }}
                                        </td>
                                        <td>
