{{-- resources/views/vouchers/receipt.blade.php --}}
@extends('layouts.app')
@section('title', isset($isEdit) ? 'تعديل ايصال استلام' : 'ايصال استلام نقدية')
...
@push('styles')
    <style>
        ...
        * {
        ...
        .acc-trigger-inline:hover,
        .acc-trigger-inline.open {
            border-bottom-color: #2563eb;
        }

        .acc-trigger-inline .arr {
            font-size: 9px;
            color: #9ca3af;
            flex-shrink: 0;
            margin-left: 4px;
        ...
        .acc-popup-inline.open {
            display: block;
        }

        .acc-search-box-inline {
            padding: 8px;
        ...
        .acc-search-box-inline input:focus {
            outline: none;
            border-color: #2563eb;
        }
        ...
        .acc-item-inline .acc-name-i mark {
            background: #fef9c3;
            color: #92400e;
            padding: 0;
        }
...
@section('content')
    <div class="voucher-page">
        <div class="voucher-wrapper">
            ...
            <div class="voucher-title-bar no-print">
                <h4>📥 {{ isset($isEdit) ? 'تعديل ايصال استلام' : 'ايصال استلام نقدية' }}</h4>
            ...
            <div class="receipt-canvas" id="receiptCanvas">
                ...
                <input type="text" class="v-field" id="refNo"
                    ...
                    value="{{ isset($detail) ? $entry->reference : $nextRef }}" {{ isset($isEdit) ? 'readonly' : '' }}>
                ...
                {{-- استلمنا من (الدائن) --}}
                <div class="acc-wrap-inline" id="creditWrap" style="top:42.5%; right:33%; width:42%; font-size:1.35vw;">
                ...
                <input type="number" class="amount-input" id="amountNum"
                    style="position:absolute; top:28%; right:2%; width:20%; font-size:1.6vw;" step="0.01" min="0"
                ...
                {{-- نقد --}}
                <div class="payoption" id="optionCash" data-method="cash" style="top:63.6%; right:7.7%;">
                    <span class="check-box"></span>
                </div>

                {{-- شيك --}}
                <div class="payoption" id="optionCheque" data-method="cheque" style="top:63.6%; right:17.8%;">
                    <span class="check-box"></span>
                </div>

                {{-- قائمة البنوك للشيك (المدين) --}}
                <div class="acc-wrap-inline" id="bankWrap"
                    ...
                    <div class="acc-trigger-inline" id="debitTrigger" onclick="toggleDropdown('debit')">
                        <span class="acc-label" id="debitLabel">— اختر البنك/الحساب —</span>
                        <span class="arr">▼</span>
                    </div>
                    ...
                    <input type="hidden" id="debitAccountId" value="">
                </div>
                ...
                {{-- الصندوق الافتراضي للنقد --}}
                <input type="hidden" id="defaultCashAccountId"
                    value="{{ isset($detail) && $detail->payment_method === 'cash' ? $detail->debit_account_id : $debitAccounts->first()?->id }}">
                <input type="hidden" id="defaultCashAccountName"
                    value="{{ isset($detail) && $detail->payment_method === 'cash' ? $detail->debitAccount->name : $debitAccounts->first()?->name }}">
    ...
    <script>
        // ══ إعدادات — تتغير تلقائياً حسب وضع الإنشاء أو التعديل ══
        @isset($isEdit)
        ...
        function numberToArabicWords(n) {
            ...
            const intPart = Math.floor(n),
                decPart = Math.round((n - intPart) * 100);
        ...
        function setPaymentMethod(method) {
            ...
            if (method === 'cash') {
                ...
                document.getElementById('debitAccountId').value = document.getElementById('defaultCashAccountId').value;
                document.getElementById('debitLabel').innerText = document.getElementById('defaultCashAccountName').value;
            } else {
                bankWrap.style.display = 'block';
                chequeNumberField.style.display = 'block';
                chequeDateField.style.display = 'block';
                document.getElementById('debitAccountId').value = '';
                document.getElementById('debitLabel').innerText = '— اختر البنك/الحساب —';
            }
        }
        ...
        function fetchAccInline(q, resultsBox, which) {
            fetch(`${SEARCH_URL}?q=${encodeURIComponent(q)}`)
                ...
                .catch(() => {
                    resultsBox.innerHTML = '<div class="acc-msg-inline">⚠️ خطأ في البحث</div>';
                });
        }
        ...
        function renderAccItems(accounts, q, resultsBox, which) {
            ...
            accounts.forEach((acc, i) => {
                ...
                const hl = q ? acc.name.replace(new RegExp(`(${q.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')})`, 'gi'),
                    '<mark>$1</mark>') : acc.name;
        ...
        document.getElementById('btnDl').addEventListener('click', function() {
            closeAllDropdowns();
            html2canvas(document.getElementById('receiptCanvas'), {
                scale: 3,
                useCORS: true,
                backgroundColor: '#fff'
            }).then(c => {
        ...
        @isset($isEdit)
            ...
            @else
                // نقد: defaultCash يتعين في setPaymentMethod بالفعل
            @endif
        @endisset
    </script>
@endsection
