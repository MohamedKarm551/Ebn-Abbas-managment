...
@extends('layouts.app')
@section('title', isset($isEdit) ? 'تعديل ايصال استلام' : 'ايصال استلام')
...
<div class="voucher-page">
    <div class="voucher-wrapper">
        ...
        <div class="receipt-canvas" id="receiptCanvas">
            ...
            <div class="acc-wrap-inline" id="creditWrap" style="top:51.5%; right:33%; width:42%; font-size:1.35vw;">
            ...
            <div style="position:absolute; top:69%; left:2%; width:5%; font-size:1.1vw; cursor:pointer; color:#2563eb; z-index:10;"
                 id="toggleBookingList" class="no-print" title="إظهار/إخفاء قائمة الحجوزات">
            ...
            {{-- الحساب المقابل (المدين) – الخزينة أو البنك – ظاهر دائماً --}}
            ...
            <input type="date" class="cheque-field-solo no-print" id="chequeDate"
                style="top:77%; right:73.8%; width:18%; display:block; direction:ltr;"
...
<script>
    ...
    document.getElementById('entryDate').addEventListener('change', function() {
        ...
        validateChequeDate(); // دائماً نتحقق
    });
    ...
    document.getElementById('btnSave').addEventListener('click', async function() {
        ...
        const debitId = document.getElementById('debitAccountId').value; // من bankWrap
        ...
        if (paymentLink) {
            finalSubject = subjectValue ? subjectValue + ' | لينك الدفع: ' + paymentLink : 'لينك الدفع: ' + paymentLink;
        }
...
// ══ جلب الحجوزات المفتوحة عند تغيير الجهة ══
let selectedBookingId = null;
let selectedBookingData = null;

// دالة تحميل الحجوزات
function loadOpenBookings(voucherType) {
    const accountId = voucherType === 'receipt' 
    ...
    fetch(`/vouchers/open-bookings?account_id=${accountId}&voucher_type=${voucherType}`)
        ...
        .then(data => {
            if (data.success && data.has_bookings) {
                ...
                data.bookings.forEach(b => {
                    ...
                    opt.dataset.originalRemaining = b.remaining;
                    opt.dataset.clientName = b.client_name;
                    select.appendChild(opt);
                });
                // ✅ إظهار القائمة فقط إذا لم نكن في وضع التعديل
                if (!IS_EDIT) {
                    wrapper.style.display = 'block';
        ...
        .catch(() => {
            ...
            wrapper.style.display = 'none'; // ✅ في التعديل نخفيها، وفي الإنشاء نعرضها
            if (!IS_EDIT) {
                wrapper.style.display = 'block';
            }
        });
...
// ══ دالة تحديث بيانات الحجز وحقل subject ══
function updateBookingDataAndSubject() {
    ...
    // تحديث نص الخيار
    selectedOption.textContent = `${clientName} - ${newRemaining.toFixed(2)} متبقي (دخول: ... )`;
...
document.getElementById('amountNum').addEventListener('input', function() {
 ...
 const bookingId = '{{ $detail->booking_id }}';
    if (bookingId && bookingId !== '' && bookingId !== 'null') {
        ...
        // تحديث subject مباشرة بدون الحاجة لـ selectedBookingId
        const subjectField = document.getElementById('subjectField');
        const clientName = '{{ optional($detail->booking)->client_name ?? "" }}';
        const originalDue = {{ optional($detail->booking)->amount_due_from_company ?? 0 }};
        const alreadyPaid = {{ optional($detail->booking)->amount_paid_by_company ?? 0 }};
...
// ══ تعديل دالة selectAcc لاستدعاء loadOpenBookings ══
const originalSelectAcc = selectAcc;
selectAcc = function(item, which) {
    originalSelectAcc(item, which);
    if ((VOUCHER_TYPE === 'receipt' && which === 'credit') || 
        (VOUCHER_TYPE === 'payment' && which === 'debit')) {
