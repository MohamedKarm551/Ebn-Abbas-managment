...
<x-slot name="title">
    {{ isset($isEdit) ? 'تعديل ايصال استلام' : 'ايصال استلام' }}
</x-slot>
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .voucher-title-bar h4 { font-size: 18px; font-weight: 700; color: #065f46; margin: 0; }
    ...
    .acc-trigger-inline .acc-label {  flex: 1;
        overflow: visible;          
        text-overflow: clip;        
        white-space: normal;        
        word-break: break-word;     
   ...
   nav, header, aside, footer,
    ...
    .receipt-canvas {
        width: 100% !important;
        max-width: 100% !important;
        /* احذف aspect-ratio واستخدم height ثابت */
...
<div class="voucher-page">
    <div class="voucher-wrapper">
        <div class="voucher-title-bar no-print">
            <h4>📥 {{ isset($isEdit) ? 'تعديل ايصال استلام' : 'ايصال استلام' }}</h4>
        ...
        <div class="receipt-canvas" id="receiptCanvas">
            ...
            {{-- استلمنا من (الدائن) - الجهة --}}
            <div class="acc-wrap-inline" id="creditWrap" style="top:50.5%; right:33%; width:42%; font-size:1.3vw;">
                <div class="acc-trigger-inline" id="creditTrigger" onclick="toggleDropdown('credit')">
                    <span class="acc-label" id="creditLabel">
                        {{ isset($detail) ? $detail->creditAccount->name : '— اختر الجهة —' }}
                    </span>
                    <span class="arr">▼</span>
                </div>
            ...
            <input type="number" class="amount-input" id="amountNum"
                style="position:absolute; top:34%; right:2%; width:20%; font-size:1.6vw;" step="0.01" min="0"
                value="{{ isset($detail) ? $detail->amount : '' }}" oninput="updateAmountArabic()">

            {{-- المبلغ كتابةً --}}
            <input type="text" class="v-field" id="amountText"
                style="top:60%; right:15%; width:75%; height:2.2vw; font-size:1.3vw;">

            {{-- وذلك عن --}}
            <input type="text" class="v-field" id="subjectField"
                style="top:69%; right:12%; width:82%; height:2.2vw; font-size:1.3vw;"
                value="{{ isset($detail) ? $detail->subject : '' }}">

            {{-- الحساب المقابل (المدين) – الخزينة أو البنك --}}
            <div class="acc-wrap-inline no-print" id="bankWrap"
                style="top:77%; right:30%; width:30%; font-size:1.1vw;">
                <div class="acc-trigger-inline" id="debitTrigger" onclick="toggleDropdown('debit')">
                    <span class="acc-label" id="debitLabel">
                        {{ isset($detail) ? $detail->debitAccount->name : '— اختر الحساب المقابل —' }}
                    </span>
                    <span class="arr">▼</span>
                </div>
                <div class="acc-popup-inline" id="debitPopup">
                    <div class="acc-search-box-inline">
                        <input type="text" placeholder="ابحث..." oninput="searchAcc(this,'debitResults')"
            ...
            {{-- تاريخ الشيك (اختياري) --}}
            <input type="date" class="v-field no-print" id="chequeDate"
            ...
            {{-- hidden input للحجز المرتبط --}}
            <input type="hidden" name="booking_id" id="bookingId" value="{{ isset($detail) ? $detail->booking_id : '' }}">

            {{-- عرض معلومات الحجز لو اتاكتشف تلقائياً --}}
            <div id="bookingInfo" style="display:none; position:absolute; top:24%; right:33%; 
                 width:42%; font-size:1.1vw; color:#1a5276; background:#d6eaf8; 
                 padding:3px 8px; border-radius:4px; text-align:right;">
                <span id="bookingInfoText"></span>
                <button type="button" onclick="clearBooking()" 
                        style="margin-right:8px; color:red; background:none; border:none; cursor:pointer;">✕</button>
            </div>
...
<script>
    ...
    function numberToArabicWords(n) {
        ...
        let words = intPart >= 1000
            ? below1000(Math.floor(intPart/1000))+' ألف'+(intPart%1000>0?' و'+below1000(intPart%1000):'')
            : below1000(intPart);
        words += ' ريال سعودي';
    ...
    function searchAcc(input, resultsId) {
        ...
        document.getElementById(resultsId).innerHTML = '<div class="acc-msg-inline">⏳ جاري البحث...</div>';
        const which = resultsId.replace('Results','');
        debounceT = setTimeout(() => fetchAccInline(input.value.trim(), document.getElementById(resultsId), which), 280);
    }
    ...
    function renderAccItems(accounts, q, resultsBox, which) {
        ...
        accounts.forEach((acc, i) => {
            ...
            item.dataset.id = acc.id; item.dataset.name = acc.name; item.dataset.code = acc.code;
    ...
    function selectAcc(item, which) {
        ...
        closeAllDropdowns();
        onAccountSelected(which, item.dataset.id, item.dataset.name);
    }
    ...
    document.getElementById('btnSave').addEventListener('click', async function () {
        ...
        const creditId = document.getElementById('creditAccountId').value;
        const debitId  = document.getElementById('debitAccountId').value;

        if (!ref || !date || !amount || !creditId || !debitId) {
            alert('يرجى تعبئة جميع الحقول المطلوبة');
            return;
        }

         // دمج اللينك مع subject
        const paymentLink = document.getElementById('link').value.trim();
        let finalSubject = document.getElementById('subjectField').value.trim();
        if (paymentLink) {
            finalSubject = finalSubject
                ? finalSubject + ' | لينك الدفع: ' + paymentLink
        ...
        } catch (e) {  alert('⚠️ خطأ في الاتصال: ' + e.message); }
        this.disabled = false;
    ...
    document.getElementById('btnDl').addEventListener('click', function () {
        ...
        html2canvas(document.getElementById('receiptCanvas'), { scale: 3, useCORS: true, backgroundColor: '#fff' })
            .then(c => {
                // إعادة الإظهار
                bankWrap.style.visibility = '';
                chequeDate.style.visibility = '';
                const a = document.createElement('a');
                a.download = `ايصال_استلام_${document.getElementById('refNo').value||'new'}.png`;
...
function onAccountSelected(type, accountId, accountName) {
    const BASE_URL = '{{ url("/vouchers/booking-by-account") }}';
    fetch(`${BASE_URL}/${accountId}`)
        .then(r => r.json())
        .then(data => {
            if (data.success && data.booking) {
                ...
                document.getElementById('bookingInfoText').innerHTML = 
                    `الحجز: <strong>${b.client_name}</strong> — 
                     الرحلة: ${b.trip_name} — 
                     المتبقي: <strong>${b.remaining}</strong> ج.م`;
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    if (bookingId && creditId) {
        onAccountSelected('credit', creditId, '');
    }
});
</script>
